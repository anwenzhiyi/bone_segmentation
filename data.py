...
class myAugmentation(object):
    ...
    def __init__(self, train_path="train", label_path="label", merge_path="merge", aug_merge_path="aug_merge",
        ...
        self.train_imgs = glob.glob(train_path + "/*." + img_type)  # glob是文件路径查找
    ...
    def splitMerge(self):
        ...
        for i in range(self.slices):
            ...
            for imgname in train_imgs:
                ...
                img_label = img[:, :, 0]
                cv2.imwrite(path_train + "/" + str(i) + "/" + midname + "." + self.img_type, img_train)
                cv2.imwrite(path_label + "/" + str(i) + "/" + midname + "." + self.img_type, img_label)
...
class dataProcess(object):
    def __init__(self, out_rows, out_cols, data_path="../deform/train", label_path="../deform/label",
                 test_path="test", npy_path="npydata", img_type="jpg"):
    ...
    def create_train_data(self):
        ...
        for imgname in imgs:
            midname = imgname[imgname.rindex("/") + 1:]  # 此处要根据实际情况改
            print(midname)
    ...
    def create_single_test_data(self, name):
        i = 0
        print('-' * 30)
        print('Creating test images...')
        print('-' * 30)
        imgs = glob.glob(self.test_path + "/" + str(name) + "." + self.img_type)
        #print(len(imgs))
        imgdatas = np.ndarray((len(imgs), self.out_rows, self.out_cols, 1), dtype=np.uint8)
        for imgname in imgs:
            midname = imgname[imgname.rindex("/") + 1:]
    ...
    def creat_and_load_single_test_image_data(self,image):
        imgdatas = np.ndarray((1, self.out_rows, self.out_cols, 1), dtype=np.uint8)
